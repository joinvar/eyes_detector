...
def resize_with_point(img, points, new_width, new_height, interp=0, letterbox=False):
    '''
    ...
    if letterbox:
        ...
        # pointx
        points[:, [0]] = points[:, [0]] * resize_ratio + dw
        # pointy
        points[:, [1]] = points[:, [1]] * resize_ratio + dh
        ...
        # pointx
        points[:, [0]] = points[:, [0]] / ori_width * new_width
        # pointy
        points[:, [1]] = points[:, [1]] / ori_height * new_height

        return img, points


def random_flip(img, points, px=0, py=0):
    '''
    ...
    if np.random.uniform(0, 1) < px:
        img = cv2.flip(img, 1)
        pointx = width - points[:, 0]
        points[:, 0] = pointx

    if np.random.uniform(0, 1) < py:
        img = cv2.flip(img, 0)
        pointy = height - points[:, 1]
        points[:, 1] = pointy
    return img, points


def random_expand(img, point, max_ratio=4, fill=0, keep_ratio=True):
    '''
