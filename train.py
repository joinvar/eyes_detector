...
from utils.eval_utils import func_evaluate, get_preds_gpu, voc_eval, parse_gt_rec
...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         inp=[x, args.class_num, args.img_size, 'train', args.multi_scale_train, args.use_mix_up, args.letterbox_resize],
                         Tout=[tf.int64, tf.float32,  tf.float32]),
...
val_dataset = val_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         inp=[x, args.class_num, args.img_size, 'val', False, False, args.letterbox_resize],
                         Tout=[tf.int64, tf.float32, tf.float32]),
    num_parallel_calls=args.num_threads
)
...
# Model definition
##################
yolo_model = yolov3(args.class_num, args.use_label_smooth, args.use_focal_loss, args.batch_norm_decay, args.weight_decay, use_static_shape=False)
with tf.variable_scope('yolov3'):
    pred_feature_maps = yolo_model.forward(image, is_training=is_training)
loss = yolo_model.compute_loss(pred_feature_maps, y_true)
y_pred = yolo_model.predict(pred_feature_maps)

# l2_loss = tf.losses.get_regularization_loss() # Refer to 49st line in file model.py
...
with tf.control_dependencies(update_ops):
    ...
    gvs = optimizer.compute_gradients(loss[0])
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        loss_total, loss_xy, loss_conf, loss_class = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        ...
        for i in trange(args.train_batch_num):
            ...
            if True:
                recall, precision = func_evaluate(sess, score_filter_op, pred_boxes_flag, pred_scores_flag, __y_pred, __y_true, args.class_num, args.nms_threshold)
                info = "\nEpoch: {}, global_step: {} | loss: total: {:.2f}, xy: {:.2f}, conf: {:.2f}, class: {:.2f} | ".format(
                        epoch, int(__global_step), loss_total.average, loss_xy.average, loss_conf.average, loss_class.average)
                info += 'Last batch: rec: {:.3f}, prec: {:.3f} | lr: {:.5e}'.format(recall, precision, __lr)
                print(info)
                logging.info(info)
                ...
                saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{} i{}'.format(epoch,int(__global_step),i),
                                   global_step=int(__global_step))
        # NOTE: this is just demo. You can set the conditions when to save the weights.
        if epoch % args.save_epoch == 0 and epoch > 0:
            # if loss_total.average <= 2.:
            saver_to_save.save(sess, args.save_dir + 'model-epoch_{}_step_{}_loss_{:.4f}_lr_{:.5e}'.format(epoch, int(__global_step), loss_total.average, __lr))
        ...
        if True:
            ...
            for j in trange(args.val_img_cnt):
                __y_true, __y_pred, __loss = sess.run([y_true, y_pred, loss],
            ...
            # info = '======> Epoch: {}, global_step: {}, lr: {:.6e} <======\n'.format(epoch, __global_step, __lr)
            info = ''
            for ii in range(args.class_num):
                npos, nd, rec, prec, ap = voc_eval(val_preds, ii, use_07_metric=args.use_voc_07_metric)
            ...
            info += 'EVAL: loss: total: {:.2f}, xy: {:.2f}, conf: {:.2f}, class: {:.2f}\n'.format(
